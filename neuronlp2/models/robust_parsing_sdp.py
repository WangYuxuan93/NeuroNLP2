...
class RobustParser(nn.Module):
    ...
    def decode(self, input_word, input_pretrained, input_char, input_pos, mask=None, bpes=None, first_idx=None, lan_id=None, leading_symbolic=0):
        """
        Args:
            input_word: Tensor
        ...
        mask_3D = (root_mask.unsqueeze(-1) * mask.unsqueeze(1))
        # (batch, seq_len, embed_size)
        embeddings = self._embed(input_word, input_pretrained, input_char, input_pos, bpes=bpes, first_idx=first_idx, lan_id=lan_id)
        # (batch, seq_len, hidden_size)
        encoder_output, _ = self._input_encoder(embeddings, mask=mask, lan_id=lan_id)
