...
class NonLinear(nn.Module):
    ...
    def reset_parameters(self):
        if self.initializer == "orthonormal":
            print ("Initializing Linear with orthonormal ...")
            ...
            print ("Initializing Linear with orthogonal ...")
            nn.init.orthogonal_(self.linear.weight)
            reset_bias_with_orthogonal(self.linear.bias)
        else:
            print ("Initializing Linear by default ...")
...
class MyLSTM(nn.Module):
    ...
    def forward(self, input, masks, initial=None):
        if self.batch_first:
            input = input.transpose(0, 1)#73*6
            masks = torch.unsqueeze(masks.transpose(0, 1), dim=2)#73*6*1
        #print(input.size())
        max_time, batch_size, _ = input.size()#sequence_length*batch_size 73*6*200
        masks = masks.expand(-1, -1, self.hidden_size)#73*6*400
        #print(masks.size())

        if initial is None:
            initial = Variable(input.data.new(batch_size, self.hidden_size).zero_())
            initial = (initial, initial)#two initial size are 6*400
        #print(initial)
        h_n = []
        c_n = []
        ...
        for layer in range(self.num_layers):
            ...
            if self.training:
                ...
                input_mask = input_mask / (1 - self.dropout_in)#size 6*200
                input_mask = torch.unsqueeze(input_mask, dim=2).expand(-1, -1, max_time).permute(2, 0, 1)#63*6*200
