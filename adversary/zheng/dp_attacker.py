...
class DPAttacker(object):
    def __init__(self, model, candidates, vocab, synonyms, filters=['word_sim', 'sent_sim', 'lm', 'train'],
        ...
        super(DPAttacker, self).__init__()
    ...
    def attack(self, tokens, tags, heads, rel_ids, sent_id=None, debug=False, cache=False):
        ...
        if debug == 3:
            #print ("tokens:\n", adv_tokens)
            print ("max allowed={}, importance rank:{}\n".format(max_mod_token, word_rank))

        # keep track of targets that have been modified
        n_valid_targets = 0

        for idx in word_rank:
            # only modify first max_mod_token valid target words
            if n_valid_targets >= max_mod_token:
                print ("--------------------------")
                print ("Modified {} word(s), breaking".format(max_mod_token))
                break
            idx = int(idx)
            ...
            if change_score[change_rank[0]] <= 0:
                #if change_score[change_rank[0]] < 0:
                chosen_cand_idx = 0
                best_cand = cands[chosen_cand_idx]
                best_c_score = change_score[chosen_cand_idx]
                #best_score = score[chosen_cand_idx]
                #new_ratio = (total_change_score + best_c_score) / (num_edit + 1)
                num_edit += 1
                total_change_score += best_c_score
                #total_score += best_score
                total_head_change += head_change[chosen_cand_idx]
                total_rel_change += rel_change[chosen_cand_idx]
                adv_tokens[idx] = best_cand
                if debug == 3:
                    print ("--------------------------")
                    print ("Idx={}({}), chosen cand:{}, no cand can make change, chosen the 1st".format(idx, tokens[idx], best_cand))
                    print ("change_scores:", *zip(cands, change_score))
                continue
            best_cand_idx = change_rank[0] #self.get_best_cand(score, change_score)
            #cand_rank = (-score).argsort()
        ...
        for x,y in zip(tokens, adv_tokens):
            if x == y:
                ...
                sent_str += y + " [ " + x + " ] "
        print ("~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        if total_head_change > 0:
            succ_flag = True
            print ("Success attack (change: head:{}, rel:{}, score:{}), adv sent:\n{}".format(
                    total_head_change, total_rel_change, total_change_score, sent_str))
        else:
            succ_flag = False
            print ("Failed attack (change: head:{}, rel:{}, score:{}), adv sent:\n{}".format(
                    total_head_change, total_rel_change, total_change_score, sent_str))
        
        adv_infos = (succ_flag, num_edit, total_change_score, total_perp_diff,
