...
from adversary.sdp.blackbox_attacker_sdp import BlackBoxAttacker
...
def attack(attacker, alg, data, network, pred_writer, adv_gold_writer, punct_set, word_alphabet, pos_alphabet, 
    ...
    for data in iterate(data, batch_size):
        ...
        for i in range(len(lengths)):
            accum_total_sent += 1
            length = lengths[i]
            adv_tokens = data['SRC'][i].copy()  # Jeffrey: 原始的word, adv_tokens就是一行句子
            adv_postags = [pos_alphabet.get_instance(w) for w in postags[i][:length]]
            # adv_heads = heads[i][:length]  # Jeffrey
            # adv_rels = rels[i][:length]
            # ******************** sdp ****************
            adv_heads = heads[i,0:length,0:length]
            adv_rels = rels[i,0:length,0:length]
...
def parse(args):
    ...
    if args.ensemble:
        ...
        model_name = os.path.join(model_path, 'model.pt')  # Jeffrey: parsing model
    ...
    adv_writer = CoNLLXWriterSDP(word_alphabet, char_alphabet, pos_alphabet, rel_alphabet)
    adv_gold_writer = CoNLLXWriterSDP(word_alphabet, char_alphabet, pos_alphabet, rel_alphabet)
