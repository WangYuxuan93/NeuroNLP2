...
from pointer.neuronlp2.io import get_logger, conllx_stacked_data
from pointer.neuronlp2.models import NewStackPtrNet
from pointer.neuronlp2 import utils
from pointer.neuronlp2.io import CoNLLXWriter
...
def train(args):
    ...
    logger.info('SEMANTIC DEPENDENCY PARSER with POINTER NETWORKS')
    logger.info('CUDA is %s', str(torch.cuda.is_available()))
    ...
    if use_gpu:
        ...
        logger.info('CUDA IS NOT AVAILABLE: %s' % str(use_gpu))
    ...
    for epoch in range(1, num_epochs + 1):
        logger.info('Epoch %d (%s, optim: %s, learning rate=%.6f, eps=%.1e, decay rate=%.2f (schedule=%d, patient=%d, decay=%d (%d, %d))): ' % (
        ...
        logger.info('train: %d loss: %.4f, arc: %.4f, type: %.4f, coverage: %.4f, time: %.2fs' %
                    (num_batches, err, err_arc, err_type, err_cov, time.time() - start_time))
        logger.info('======EVALUATING PERFORMANCE ON DEV======')
        ...
        logger.info('CUR DEV %d: ucorr: %d, lcorr: %d, tot_gold: %d, tot_pred: %d, Uprec: %.2f%%, Urec: %.2f%%, Lprec: %.2f%%, Lrec: %.2f%%, UF1: %.2f%%, LF1: %.2f%%' % (
        ...
        lasted_time_test = 100.0
        lasted_time_test2 = 100.0
        if dev_bestLF1 < dev_LF1:
            ...
            logger.info('======EVALUATING PERFORMANCE ON TEST======')
        ...
        logger.info('----------------------------------------------------------------------------------------------------------------------------')
        logger.info('TIME DEV: %.2f,NUM SENTS DEV: %d,SPEED DEV: %.2f' %
                    (lasted_time_dev, dev_total_inst, dev_total_inst / lasted_time_dev))
        logger.info('DEV: Uprec: %.2f%%, Urec: %.2f%%, Lprec: %.2f%%, Lrec: %.2f%%, UF1: %.2f%%, LF1: %.2f%% (epoch: %d)' %
                    (dev_bestUprecision, dev_bestUrecall, dev_bestLprecision, dev_bestLrecall, dev_bestUF1, dev_bestLF1, best_epoch))
        logger.info('----------------------------------------------------------------------------------------------------------------------------')
        logger.info('TIME TEST: %.2f,NUM SENTS TEST: %d,SPEED TEST: %.2f' %
        ...
        logger.info('TEST: ucorr: %d, lcorr: %d, tot_gold: %d, tot_pred: %d, Uprec: %.2f%%, Urec: %.2f%%, Lprec: %.2f%%, Lrec: %.2f%%, '
                    'UF1: %.2f%%, LF1: %.2f%% (epoch: %d)' % (test_ucorrect, test_lcorrect, test_total_gold, test_total_pred, test_Uprecision,
                                                              test_Urecall, test_Lprecision, test_Lrecall, test_UF1, test_LF1, best_epoch))
    ...
    # # *********** jeffrey: ood ****************
    # logger.info('======EVALUATING PERFORMANCE ON TEST 2======')
    # # pred_filename = 'tmp/%spred_test%d' % (str(uid), epoch)
    ...
    #
    # logger.info('----------------------------------------------------------------------------------------------------------------------------')
    # logger.info('TIME TEST2: %.2f, NUM SENTS TEST: %d, SPEED TEST2: %2.f' % (lasted_time_test2, test2_total_inst, test2_total_inst / lasted_time_test2))
    # logger.info('TEST2: ucorr: %d, lcorr: %d, tot_gold: %d, tot_pred: %d, Uprec: %.2f%%, Urec: %.2f%%, Lprec: %.2f%%, Lrec: %.2f%%, '
    #             'UF1: %.2f%%, LF1: %.2f%% (epoch: %d)' % (test2_ucorrect, test2_lcorrect, test2_total_gold, test2_total_pred, test2_Uprecision,
    #                                                       test2_Urecall, test2_Lprecision, test2_Lrecall, test2_UF1, test2_LF1, best_epoch))
    # logger.info('====================F========================================================================================================')
...
def parse(args):
    ...
    with torch.no_grad():
        logger.info('Parsing...')
        eval(network=network,data_test=data_test,batch_size=args.batch_size,word_alphabet=word_alphabet,
             lemma_alphabet=lemma_alphabet,pos_alphabet=pos_alphabet,beam=args.beam,punct_set=punct_set,best_epoch=0,logger=logger)
    logger.info("Parsing ends!")
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--train_mode', default="train",choices=["train","parse"],help='train mode or parse mode')
