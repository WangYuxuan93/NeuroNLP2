...
class Attacker(object):
    def __init__(self, gpu, config, common_config,pointer, output_dir):
        self.tasks = load_config(config)
        self.pointer = load_config(pointer)
        self.common_dict = load_config(common_config)
        self.common_dict["gpu"] = gpu
        self.output_dir = output_dir
        self.src = "source "+self.common_dict["src_path"]+" ; "  # activate the env
...
args_parser.add_argument('pointer_config',type=str, help="pointer config")
...
for config in configs:
    attacker = Attacker(args.gpu, config, args.common_config,args.pointer_config, args.output_dir)
