...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, lemma_alphabet, max_size=None, normalize_digits=True, prior_order='deep_first', use_gpu=False,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        if use_gpu:
            ...
            lemma_single_t = lemma_single_t.cuda()
            lengths_e_t = lengths_e_t.cuda()
            ...
            masks_d_t = masks_d_t.cuda()
            lengths_d_t = lengths_d_t.cuda()
            previous = previous.cuda()
            next = next.cuda()

        data_variable.append(
            ((words, lemmas, chars, pos, heads, types, masks_t, single_t, lemma_single_t, lengths_e_t), (stacked_heads, children, siblings, stacked_types, skip_connect, previous, next, masks_d_t,
                                                                                                    lengths_d_t)))
